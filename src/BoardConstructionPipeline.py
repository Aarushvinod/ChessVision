...
class ImageBoardRecognizer:
    ...
    def __init__(self, pieces_name: str, board_name: str):
        ...
        #Deferring imports to isolate any errors that arise with detectron2
        ...
        merge_path = Path(__file__).resolve().parents[1] / "detectron2" / "projects/PointRend/configs/InstanceSegmentation/pointrend_rcnn_X_101_32x8d_FPN_3x_coco.yaml"
        cfg.merge_from_file(merge_path)
        cfg.MODEL.DEVICE = "cpu" if not GPU.is_available() else "cuda"
        cfg.MODEL.WEIGHTS = f"src/models/{board_name}"
    ...
    def run_board_detection(self, **kwargs):
        ...
        if kwargs.get('save'):
            ...
            cv2.imwrite("tests/board_detection.jpg", copy_image)
        return final_mask
